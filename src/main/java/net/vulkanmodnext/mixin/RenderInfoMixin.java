...
public abstract class RenderInfoMixin implements RenderInfo {
    ...
    @Override
    public RenderChunk vulkanmodnext$chunk() {
        return this.renderChunk;
    }

    @Override
    public EnumFacing vulkanmodnext$facing() {
        return this.facing;
    }

    @Override
    public void vulkanmodnext$reset(RenderChunk chunk, EnumFacing facingIn, int counterIn) {
        this.renderChunk = chunk;
        this.facing = facingIn;
        this.counter = counterIn;
        this.setFacing = 0;
        this.vulkanmodnext$slotPlusOne = 0;
    }

    @Override
    public void vulkanmodnext$setDirection(byte parentFacing, EnumFacing step) {
        this.setFacing = (byte) (this.setFacing | parentFacing | 1 << step.ordinal());
    }

    @Override
    public boolean vulkanmodnext$hasDirection(EnumFacing direction) {
        return (this.setFacing & 1 << direction.ordinal()) > 0;
    }

    @Override
    public byte vulkanmodnext$facingMask() {
