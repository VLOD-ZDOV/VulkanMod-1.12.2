 ...
 * VulkanModNext line to the F3 debug screen.
 *
...
public final class VulkanDemoOverlay {
    ...
    private static final Logger LOGGER = LogManager.getLogger("VulkanModNext/Overlay");
     ...
     * like any other switch — or with -Dvulkanmodnext.overlay=true for a run
    ...
    public void onDebugText(RenderGameOverlayEvent.Text event) {
        if (Minecraft.getMinecraft().gameSettings.showDebugInfo) {
            event.getRight().add("");
            event.getRight().add("VulkanModNext: " + bridge.gpuSummary());
            event.getRight().add("VulkanModNext: " + bridge.chunkMirrorStats()
                    + (Boolean.TRUE.equals(interop) ? ", zero-copy on" : ""));
            event.getRight().add("VulkanModNext: " + TerrainHooks.stats());
        }
    }
    ...
    private static boolean showOverlay() {
        return Boolean.getBoolean("vulkanmodnext.overlay") || VulkanConfig.isOverlayEnabled();
    }
