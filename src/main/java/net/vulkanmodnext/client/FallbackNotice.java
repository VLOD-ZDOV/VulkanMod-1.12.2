...
public final class FallbackNotice {
    ...
    public void onDebugText(RenderGameOverlayEvent.Text event) {
        if (Minecraft.getMinecraft().gameSettings.showDebugInfo) {
            event.getRight().add("");
            event.getRight().add("VulkanModNext: renderer off, vanilla OpenGL");
            event.getRight().add("VulkanModNext: " + reason);
        }
    }
    ...
    private static String describe(Throwable failure, int limit) {
        ...
        for (Throwable link = failure; link != null && link.getCause() != link; link = link.getCause()) {
            if (link instanceof net.vulkanmodnext.VulkanUnavailableException) {
                root = link;
                break;
            }
            root = link;
