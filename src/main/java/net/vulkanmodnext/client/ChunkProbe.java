...
import net.vulkanmodnext.mixin.RenderChunkAccessor;
import net.vulkanmodnext.mixin.RenderGlobalAccessor;
...
public final class ChunkProbe {
    ...
    private static final Logger LOGGER = LogManager.getLogger("VulkanModNext/Probe");

    private static final KeyBinding KEY = new KeyBinding(
            "key.vulkanmodnext.probe", Keyboard.KEY_NONE, "key.categories.vulkanmodnext");
    ...
    private static void probe(Minecraft mc) {
        ...
        ViewFrustum frustum = ((RenderGlobalAccessor) mc.renderGlobal).vulkanmodnext$viewFrustum();
        ...
        for (RenderChunk chunk : frustum.renderChunks) {
            ...
            int stamp = ((RenderChunkAccessor) chunk).vulkanmodnext$frameIndex();
            if (stamp > newest) {
                newest = stamp;
            }
        }
        for (RenderChunk chunk : frustum.renderChunks) {
            ...
            if (((RenderChunkAccessor) chunk).vulkanmodnext$frameIndex() == newest) {
                reached++;
            }
        ...
        try {
            java.util.Set<RenderChunk> waiting =
                    ((RenderGlobalAccessor) mc.renderGlobal).vulkanmodnext$chunksToUpdate();
