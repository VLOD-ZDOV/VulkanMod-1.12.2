...
public interface RenderInfo {
    ...
    RenderChunk vulkanmodnext$chunk();

    EnumFacing vulkanmodnext$facing();

    /** Puts a pooled record back into the state a fresh one would have. */
    void vulkanmodnext$reset(RenderChunk chunk, EnumFacing facing, int counter);
     ...
     * the way it came.
     */
    void vulkanmodnext$setDirection(byte parentFacing, EnumFacing step);

    boolean vulkanmodnext$hasDirection(EnumFacing direction);

    byte vulkanmodnext$facingMask();
