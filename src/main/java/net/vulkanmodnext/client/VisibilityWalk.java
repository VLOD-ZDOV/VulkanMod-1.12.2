...
public final class VisibilityWalk {
    ...
    private static final boolean VERIFY =
            Boolean.getBoolean("vulkanmodnext.verifyWalk");
    ...
    public RenderInfo seed(Object owner, RenderChunk chunk, int slot, int x, int y, int z) {
        ...
        record.vulkanmodnext$setGridSlot(slot);
        stamp[slot] = epoch;
    ...
    public boolean iterate(Object owner, RenderChunk[] chunks, Frustum camera, List out,
        ...
        while (head < tail) {
            ...
            int emitSlot = info.vulkanmodnext$gridSlot();
            if (emitSlot < 0) {
            ...
            EnumFacing entered = info.vulkanmodnext$facing();
            byte mask = info.vulkanmodnext$facingMask();
            ...
            CompiledChunk compiled = info.vulkanmodnext$chunk().getCompiledChunk();
            if (!compiled.isEmpty()) {
            ...
            for (int face = 0; face < 6; face++) {
                ...
                child.vulkanmodnext$setDirection(mask, FACES[face]);
                ...
                child.vulkanmodnext$setGridSlot(slot);
    ...
    public int visibleOrder(RenderInfo info) {
        int slot = info.vulkanmodnext$gridSlot();
    ...
    public void applyArrivals() {
        ...
        while ((chunk = CompiledArrivals.poll()) != null) {
            ...
            RenderInfo info = visibleSection(position.getX() >> 4, position.getY() >> 4,
                    position.getZ() >> 4);
            if (info == null || info.vulkanmodnext$chunk() != chunk) {
                continue;
            }
            if (blocks) {
