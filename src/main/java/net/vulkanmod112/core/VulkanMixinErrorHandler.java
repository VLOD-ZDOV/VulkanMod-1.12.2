...
public class VulkanMixinErrorHandler implements IMixinErrorHandler {
    ...
    private static final String OUR_PACKAGE = "net.vulkanmod112.mixin.";
    ...
    private static ErrorAction handle(IMixinInfo mixin, Throwable th, ErrorAction action) {
        try {
            ...
            if (VulkanPatchGroups.isEssential(group)) {
                System.out.println("[VulkanMod112] " + reason + ". This patch is part of the "
                        + "renderer itself and cannot be switched off separately. Set "
                        + "terrainEnabled=false in config/vulkanmod112.cfg to start without it, "
                        + "and please report this with the whole of logs/latest.log.");
                return action;
            }
            VulkanPatchState.quarantine(group, reason);
            System.out.println("[VulkanMod112] " + reason);
            System.out.println("[VulkanMod112] " + VulkanPatchGroups.title(group)
            ...
            System.out.println("[VulkanMod112] Could not record the failed patch. " + failure);
        }
        return action;
    }
