...
public abstract class EntitySectionsMixin {
    ...
    @Unique
    @SuppressWarnings("rawtypes")
    private final List vulkanmod112$withEntities = new ArrayList(64);
    ...
    private static final boolean VULKANMOD112$VERIFY =
            Boolean.getBoolean("vulkanmod112.verifyEntitySections");
    ...
    private List vulkanmod112$sectionsWithEntities(RenderGlobal self) {
        ...
        List out = vulkanmod112$withEntities;
        out.clear();
        walk.beginPick();
        ...
        for (int i = 0, size = loaded.size(); i < size; i++) {
            ...
            RenderInfo info = walk.visibleSection(entity.chunkCoordX, entity.chunkCoordY,
                    entity.chunkCoordZ);
            if (info != null && walk.pick(info.vulkanmod112$gridSlot())) {
                out.add(info);
            }
        }
        vulkanmod112$sortByVisibleOrder(out, walk);
        VanillaFrame.countEntitySections(out.size(), full.size());
        if (VULKANMOD112$VERIFY) {
            vulkanmod112$verifyEntities(full, out);
        }
        return out;
    }
    ...
    @SuppressWarnings("rawtypes")
    private List vulkanmod112$sectionsWithTileEntities(RenderGlobal self) {
        List full = renderInfos;
        VisibilityWalk walk = vulkanmod112$index(full);
        if (walk == null) {
            return full;
        ...
        if (VULKANMOD112$VERIFY) {
            vulkanmod112$verifyTileEntities(full, out);
        }
        return out;
    }
    ...
    @Unique
    @SuppressWarnings({"rawtypes", "unchecked"})
    private static void vulkanmod112$sortByVisibleOrder(List out, VisibilityWalk walk) {
    ...
    @Unique
    @SuppressWarnings("rawtypes")
    private void vulkanmod112$verifyEntities(List full, List out) {
        int expected = 0;
        int missing = 0;
    ...
    @Unique
    @SuppressWarnings("rawtypes")
    private void vulkanmod112$verifyTileEntities(List full, List out) {
        int expected = 0;
        int missing = 0;
        for (int i = 0; i < full.size(); i++) {
            RenderInfo info = (RenderInfo) full.get(i);
            if (info.vulkanmod112$chunk().getCompiledChunk().getTileEntities().isEmpty()) {
                continue;
            }
            expected++;
            if (!out.contains(info)) {
