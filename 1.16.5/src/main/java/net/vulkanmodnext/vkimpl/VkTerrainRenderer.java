...
final class VkTerrainRenderer {
    ...
    private void transferSharedImages(MemoryStack stack, VkCommandBuffer cmd, boolean release) {
        ...
        VkImageMemoryBarrier.Buffer barriers = VkImageMemoryBarrier.callocStack(2, stack);
    ...
    VkTerrainRenderer(VkContext ctx) {
        this.ctx = ctx;
    }
    ...
    private void recordAtlasUpload(MemoryStack stack) {
        ...
        VkBufferImageCopy.Buffer copy = VkBufferImageCopy.callocStack(regions, stack);
    ...
    private void createQueryPool(MemoryStack stack) {
        ...
        VkQueueFamilyProperties.Buffer families =
                VkQueueFamilyProperties.mallocStack(familyCount.get(0), stack);
        ...
        VkQueryPoolCreateInfo info = VkQueryPoolCreateInfo.callocStack(stack)
    ...
    private void beginFrame(float[] mvp, VkChunkMirror mirror) {
        try (MemoryStack stack = push()) {
            ...
            VkClearValue.Buffer clears = VkClearValue.callocStack(2, stack);
            clears.get(0).color()
                    .float32(0, 0.0f).float32(1, 0.0f).float32(2, 0.0f).float32(3, 0.0f);
            clears.get(1).depthStencil().depth(1.0f).stencil(0);

            VkRenderPassBeginInfo rpBegin = VkRenderPassBeginInfo.callocStack(stack)
            ...
            org.lwjgl.vulkan.VkViewport.Buffer viewport = org.lwjgl.vulkan.VkViewport.callocStack(1, stack);
            viewport.get(0).x(0).y(0).width(width).height(height).minDepth(0.0f).maxDepth(1.0f);
            vkCmdSetViewport(commandBuffer, 0, viewport);
            VkRect2D.Buffer scissor = VkRect2D.callocStack(1, stack);
            scissor.get(0).extent(VkExtent2D.callocStack(stack).width(width).height(height));
    ...
    private boolean ensureSpriteVertexCapacity(int slot, long bytes) {
        ...
        try (MemoryStack stack = push()) {
            ...
            if (traced) {
                ...
                alloc.pNext(org.lwjgl.vulkan.VkMemoryAllocateFlagsInfo.callocStack(stack)
    ...
    private void writeSpriteSet(int slot, long view, long sampler) {
        ...
        vkDeviceWaitIdle(device());
        try (MemoryStack stack = push()) {
            VkDescriptorImageInfo.Buffer info = VkDescriptorImageInfo.callocStack(1, stack);
            ...
            write.get(0)
                    ...
                    .dstSet(spriteSets[slot]).dstBinding(0)
    ...
    private boolean renderTranslucent(int[] chunks, int chunkCount, float[] mvp,
        ...
        try (MemoryStack stack = push()) {
            ...
            VkClearValue.Buffer clears = VkClearValue.callocStack(1, stack);
    ...
    private void recordLightmapUpload(MemoryStack stack) {
        VkImageMemoryBarrier.Buffer toTransfer = VkImageMemoryBarrier.callocStack(1, stack);
        toTransfer.get(0)
                .sType(VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER)
        ...
        VkImageMemoryBarrier.Buffer toShader = VkImageMemoryBarrier.callocStack(1, stack);
        toShader.get(0)
                .sType(VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER)
                .srcAccessMask(VK_ACCESS_TRANSFER_WRITE_BIT)
                .dstAccessMask(VK_ACCESS_SHADER_READ_BIT)
    ...
    private void ensureBaseResources() {
        ...
        try (MemoryStack stack = push()) {
            ...
            VkExportSemaphoreCreateInfo export = VkExportSemaphoreCreateInfo.callocStack(stack)
            ...
            VkSemaphoreCreateInfo semInfo = VkSemaphoreCreateInfo.callocStack(stack)
    ...
    private long createAtlasSampler(MemoryStack stack) {
        VkSamplerCreateInfo info = VkSamplerCreateInfo.callocStack(stack)
    ...
    private void createDescriptorInfrastructure(MemoryStack stack) {
        ...
        VkSamplerCreateInfo sceneSamplerInfo = VkSamplerCreateInfo.callocStack(stack)
                .sType(VK_STRUCTURE_TYPE_SAMPLER_CREATE_INFO)
        ...
        VkDescriptorSetLayoutBinding.Buffer bindings =
                VkDescriptorSetLayoutBinding.callocStack(tracing ? 7 : 6, stack);
        ...
        VkDescriptorPoolSize.Buffer poolSizes = VkDescriptorPoolSize.callocStack(tracing ? 4 : 3, stack);
        ...
        VkDescriptorSetAllocateInfo setInfo = VkDescriptorSetAllocateInfo.callocStack(stack)
    ...
    private void ensureDrawBatchCapacity(int draws) {
        ...
        try (MemoryStack stack = push()) {
            ...
            VkWriteDescriptorSet.Buffer writes =
                    VkWriteDescriptorSet.callocStack(drawDescriptorSets.length, stack);
            for (int i = 0; i < drawDescriptorSets.length; i++) {
                VkDescriptorBufferInfo.Buffer bufferInfo = VkDescriptorBufferInfo.callocStack(1, stack);
                bufferInfo.get(0).buffer(drawBatchBuffers[i]).offset(0).range(drawCommandOffset);
                writes.get(i)
                        .sType(VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET)
                        .dstSet(drawDescriptorSets[i]).dstBinding(2)
    ...
    private void writeDescriptors() {
        ...
        try (MemoryStack stack = push()) {
            VkDescriptorImageInfo.Buffer atlasInfo = VkDescriptorImageInfo.callocStack(1, stack);
            atlasInfo.get(0)
                    .sampler(atlasSampler)
                    .imageView(atlasView)
                    .imageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
            VkDescriptorImageInfo.Buffer lightmapInfo = VkDescriptorImageInfo.callocStack(1, stack);
            lightmapInfo.get(0)
            ...
            VkDescriptorImageInfo.Buffer sceneColorInfo = VkDescriptorImageInfo.callocStack(1, stack);
            boolean sceneReady = colorView != 0 && depthView != 0 && sceneWanted();
            sceneColorInfo.get(0)
                    .sampler(sceneSampler)
                    .imageView(sceneReady ? colorView : atlasView)
                    .imageLayout(VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
            VkDescriptorImageInfo.Buffer sceneDepthInfo = VkDescriptorImageInfo.callocStack(1, stack);
            ...
            VkWriteDescriptorSet.Buffer writes = VkWriteDescriptorSet.callocStack(drawDescriptorSets.length * 6, stack);
            for (int i = 0; i < drawDescriptorSets.length; i++) {
                VkDescriptorBufferInfo.Buffer bufferInfo = VkDescriptorBufferInfo.callocStack(1, stack);
                bufferInfo.get(0).buffer(drawBatchBuffers[i]).offset(0).range(drawCommandOffset);
                // Three sets per frame in flight, one per layer, and they all
                // read the same frame constants.
                VkDescriptorBufferInfo.Buffer frameInfo = VkDescriptorBufferInfo.callocStack(1, stack);
                ...
                writes.get(write)
                        .sType(VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET)
                        .dstSet(drawDescriptorSets[i]).dstBinding(0)
                        .descriptorType(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER).pImageInfo(atlasInfo);
                writes.get(write + 1)
                        .sType(VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET)
                        .dstSet(drawDescriptorSets[i]).dstBinding(1)
                ...
                writes.get(write + 3)
                        .sType(VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET)
                        .dstSet(drawDescriptorSets[i]).dstBinding(3)
                        .descriptorType(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER).pBufferInfo(frameInfo);
                writes.get(write + 4)
                        .sType(VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET)
                        .dstSet(drawDescriptorSets[i]).dstBinding(4)
                        .descriptorType(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
                        .pImageInfo(waterSet ? sceneColorInfo : atlasInfo);
                writes.get(write + 5)
                        .sType(VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET)
                        .dstSet(drawDescriptorSets[i]).dstBinding(5)
                        .descriptorType(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER)
    ...
    private void decideColourDepth(MemoryStack stack) {
        ...
        org.lwjgl.vulkan.VkImageFormatProperties props =
                org.lwjgl.vulkan.VkImageFormatProperties.callocStack(stack);
    ...
    private void createRenderPass(MemoryStack stack) {
        ...
        VkAttachmentReference depthRef = VkAttachmentReference.callocStack(stack)
        ...
        VkRenderPassCreateInfo rpInfo = VkRenderPassCreateInfo.callocStack(stack)
    ...
    private void createTranslucentRenderPass(MemoryStack stack) {
        ...
        VkAttachmentReference creatureDepthRef = VkAttachmentReference.callocStack(stack)
                .attachment(1).layout(VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL);
    ...
    private void createPipelineSet(MemoryStack stack, boolean rayQuery) {
        ...
        VkVertexInputBindingDescription.Buffer binding = VkVertexInputBindingDescription.callocStack(2, stack);
        ...
        VkVertexInputAttributeDescription.Buffer attrs =
                VkVertexInputAttributeDescription.callocStack(packed ? 4 : 5, stack);
        if (packed) {
        ...
        VkPipelineRasterizationStateCreateInfo raster = VkPipelineRasterizationStateCreateInfo.callocStack(stack)
        ...
        VkPipelineDynamicStateCreateInfo dynamic = VkPipelineDynamicStateCreateInfo.callocStack(stack)
                .sType(VK_STRUCTURE_TYPE_PIPELINE_DYNAMIC_STATE_CREATE_INFO)
                .pDynamicStates(stack.ints(VK_DYNAMIC_STATE_VIEWPORT, VK_DYNAMIC_STATE_SCISSOR));

        VkPushConstantRange.Buffer pushRange = VkPushConstantRange.callocStack(1, stack);
        pushRange.get(0)
                .stageFlags(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT)
                .offset(0)
        ...
        VkSpecializationMapEntry.Buffer specEntry = VkSpecializationMapEntry.callocStack(2, stack);
        ...
        VkGraphicsPipelineCreateInfo.Buffer pipelineInfo =
                VkGraphicsPipelineCreateInfo.callocStack(TERRAIN_PIPELINES.length, stack);
        for (int variant = 0; variant < TERRAIN_PIPELINES.length; variant++) {
            TerrainPipeline spec = TERRAIN_PIPELINES[variant];
            VkSpecializationInfo specInfo = VkSpecializationInfo.callocStack(stack)
            ...
            VkPipelineColorBlendStateCreateInfo blend = VkPipelineColorBlendStateCreateInfo.callocStack(stack)
    ...
    private void createSpriteResources(MemoryStack stack) {
        ...
        VkVertexInputAttributeDescription.Buffer attrs =
                VkVertexInputAttributeDescription.callocStack(4, stack);
        attrs.get(0).location(0).binding(0).format(VK_FORMAT_R32G32B32_SFLOAT).offset(0);
    ...
    private void ensureTargets(int fbWidth, int fbHeight) {
        ...
        try (MemoryStack stack = push()) {
            ...
            VkFramebufferCreateInfo fbInfo = VkFramebufferCreateInfo.callocStack(stack)
            ...
            VkBufferCreateInfo rbInfo = VkBufferCreateInfo.callocStack(stack)
                    .sType(VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO)
                    .size(readbackSize)
            ...
            VkMemoryRequirements rbReq = VkMemoryRequirements.mallocStack(stack);
            vkGetBufferMemoryRequirements(device(), readbackBuffer, rbReq);
            VkMemoryAllocateInfo rbAlloc = VkMemoryAllocateInfo.callocStack(stack)
                    .sType(VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO)
    ...
    private void createExportedTarget(MemoryStack stack, int format, int usage, int aspect, long[] out) {
        VkExternalMemoryImageCreateInfo external = VkExternalMemoryImageCreateInfo.callocStack(stack)
        ...
        VkMemoryDedicatedAllocateInfo dedicated = VkMemoryDedicatedAllocateInfo.callocStack(stack)
                .sType(VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO)
                .image(image);
        VkExportMemoryAllocateInfo export = VkExportMemoryAllocateInfo.callocStack(stack)
    ...
    private void createLightmapResources() {
        try (MemoryStack stack = push()) {
            ...
            for (int i = 0; i < framesInFlight; i++) {
                ...
                alloc = VkMemoryAllocateInfo.callocStack(stack)
                        .sType(VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO)
                        .allocationSize(req.size())
                        .memoryTypeIndex(findMemoryType(stack, req.memoryTypeBits(),
                                VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT));
    ...
    private void createSampledImage(MemoryStack stack, int imgWidth, int imgHeight, ByteBuffer[] levels,
        ...
        for (int level = 0; level < mipLevels; level++) {
            ...
            VkMemoryRequirements sreq = VkMemoryRequirements.mallocStack(stack);
            vkGetBufferMemoryRequirements(device(), stagingBuffers[level], sreq);
            VkMemoryAllocateInfo salloc = VkMemoryAllocateInfo.callocStack(stack)
                    .sType(VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO)
        ...
        VkSubmitInfo empty = VkSubmitInfo.callocStack(stack).sType(VK_STRUCTURE_TYPE_SUBMIT_INFO);
